  ...
  for (w in 1:model$K) {
    if (!is.null(xs)) { # contextual
      ...
      if(balanced == 'ipw'){
        ...
        regr <- cv.glmnet(model$X[[w]], model$y[[w]], alpha = 0, weights = W) 
      ...
      } else if (balanced == 'stabilized') {
        ps_hj <- model$ps[[w]]/sum(model$ps[[w]])
        W <- 1/ps_hj # balancing weights
        regr <- cv.glmnet(model$X[[w]], model$y[[w]], alpha = 0, weights = W) 
...
# covariate matrix
xmat <- apply(as.matrix(df_working[, predv_cols]), 2, function(x) { 
  x * as.matrix(df_working[, predv_cols])
}, simplify = FALSE)
...
muxs_l <- lapply(0:ncol(wmat), function(x) {
  ...
  predict(lmr, newx = XX, s = lmr$lambda.min)
})
...
floor_start <- 5
floor_decay <- 0.9
batch_sizes <- c(200, 200, 200, 200, 200)
...
for(i in 1:niter){
  ...
  # resample from data
  shuffle <- sample(1:nrow(xmat), A, replace = TRUE)
  xs <- xmat[shuffle,]
  muxs <- muXXs[shuffle,]
  ys <- muxs + matrix(runif(A*K, -0.3, 0.3), nrow = A, ncol = K)
  ...
  ws_opt <- apply(predict(cf.priority, xmat)$predictions, 1, which.max)
  ...
  # linear stabilized balanced TS
  results <- run_experiment(ys, floor_start, floor_decay, batch_sizes, xs, balanced = 'stabilized')
  
  # on-policy assignment
  out_l[[4]][i,1] <- mean(apply(muxs, 1, which.max) == results$ws)
  ...
  ws_opt <- apply(predict(cf.priority, xmat)$predictions, 1, which.max)
  out_l[[4]][i,2] <- mean(muXXs[cbind(1:length(ws_opt), ws_opt)])
  ...
  ws_opt <- apply(predict(cf.priority, xmat)$predictions, 1, which.max)
  out_l[[4]][i,3] <- mean(muXXs[cbind(1:length(ws_opt), ws_opt)])
  
  # regret
  out_l[[4]][i,4] <- mean(muxs[cbind(1:length(results$ws), apply(muxs, 1, which.max))] - 
                            muxs[cbind(1:length(results$ws), results$ws)])
  
  # mean inverse probability for on-assignment
  out_l[[4]][i,5] <- mean(1/probs_r[cbind(1:A, apply(muxs, 1, which.max))])
...
rownames(results_mat) <- c('uniform', 'LinTS', 'BLTS', 'hjBLTS')
