...
gg1 <- ggplot(df_grid[which(df_grid$level == 'Interactions: Respondent x headline'),], 
  ...
  guides(color=guide_legend(title='Headline level', reverse = TRUE, 
         ...
         fill=guide_legend(title='Headline level', reverse = TRUE, 
                           title.position = 'top', title.hjust = 0.5)) + 
  geom_text(aes(x = position_x, y = term, 
                label = round(Probabilities,3)),
...
ggsave('../figures/figure1.pdf', height = 6, width = 8)

write.csv(x = df_grid[,c('estimate', 'std.error', 'Probabilities', 'Headline', 'Respondent', 'level')],
          file = '../figures/figure1_source.csv', 
          row.names = FALSE
)
```
...
ggsave('../figures/figure1_alt.pdf', height = 6, width = 8)

write.csv(x = df_grid[,c('estimate', 'std.error', 'Probabilities', 'Headline', 'Respondent', 'level')],
          file = '../figures/figure1_alt_source.csv', 
          row.names = FALSE
)
```
...
models <- lapply(models, function(x){
  ...
  x$tidy$conf.low = x$tidy$estimate - 1.96*x$tidy$std.error
  x$tidy$conf.high = x$tidy$estimate + 1.96*x$tidy$std.error
  x$tidy[x$tidy$Type != 2, c('p.value', 'conf.low', 'conf.high')] <- NA
...
row9 <- '\\cmidrule(lr){2-8}'
row15 <- '\\multicolumn{4}{l}{\\textbf{Gender}} \\rule{0pt}{1.2\\normalbaselineskip}\\\\'
row21 <- '\\cmidrule(lr){2-8}'
row27 <- '\\multicolumn{4}{l}{\\textbf{Supports governing party}} \\rule{0pt}{1.2\\normalbaselineskip}\\\\'
row33 <- '\\cmidrule(lr){2-8}'
row39 <- '\\multicolumn{4}{l}{\\textbf{Digital literacy index}} \\rule{0pt}{1.2\\normalbaselineskip}\\\\'
row45 <- '\\cmidrule(lr){2-8}'
row51 <- '\\multicolumn{4}{l}{\\textbf{Scientific knowledge index}} \\rule{0pt}{1.2\\normalbaselineskip}\\\\'
row57 <- '\\cmidrule(lr){2-8}'

attr(rows, 'position') <- c(1,2,3,9,15,21,27,33,39,45,51,57)
...
xx <- modelsummary(models, 
                   ...
                   col.names = NULL,
                   # fmt = NULL,
                   statistic = c('({std.error})', '$p =$ {p.value}', '[{conf.low}, {conf.high}]'),
...
# drop CIs and p-values for all but differences
...
writeLines(xx, '../tables/table1.tex')
...
ggplot(all_scores, aes(x = estimate, y =term, color=`Response measure`, 
  ...
  geom_point(data = all_scores %>% filter(Channel != 'Any'),
             ...
             size = 2, fill = 'white') + # change size, fill here
  geom_point(data = all_scores %>% filter(Channel == 'Any'), 
             size = 2, fill = 'white') + # change size, fill here
...
ggsave('../figures/figure2.pdf', width = 8, height = 4)

xx <- as.matrix(apply(all_scores[,
                         c('estimate', 'std.error', 'Channel', 'term', 
                           'Response measure')], 
            2, function(x) gsub('\n', ' ', 
                    x,
                     perl = TRUE,
...
write.csv(x = xx,
          file = '../figures/figure2_source.csv', 
          row.names = FALSE
)
```
...
models <- lapply(models, function(x){
  class(x) <- "modelsummary_list"
  x$tidy$p.value <- x$tidy[,x$pval]
  x$tidy$conf.low = x$tidy$estimate - 1.96*x$tidy$std.error
  x$tidy$conf.high = x$tidy$estimate + 1.96*x$tidy$std.error
  x$tidy[x$tidy$term == 'Control',
         c('p.value', 'conf.low', 'conf.high')] <- NA
  x
})
...
row12 <- '\\multicolumn{4}{l}{\\textbf{Respondent treatment effects}} \\rule{0pt}{1.2\\normalbaselineskip}\\\\'
row29 <- '\\hline'

attr(rows, 'position') <- c(1,2,3,12,29)
...
writeLines(xx, '../tables/table2.tex')
...
row3 <- paste0('\\cmidrule(lr){2-2} \\cmidrule(lr){3-5} \\cmidrule(lr){6-8} \\multicolumn{4}{l}{\\textbf{Optimal assignment == Accuracy nudge (n = ', prettyNum(table(optimal_assignment)[1], big.mark = ","),
               ')}} \\rule{0pt}{1.2\\normalbaselineskip}\\\\')
row10 <- '\\cmidrule(lr){2-8}'
row15 <- paste0('\\multicolumn{4}{l}{\\textbf{Optimal assignment == Facebook tips (n = ', prettyNum(table(optimal_assignment)[2], big.mark = ","),
                ')}} \\rule{0pt}{1.2\\normalbaselineskip}\\\\')
row22 <- '\\cmidrule(lr){2-8}'

attr(rows, 'position') <- c(1,2,3,10,15,21)
...
writeLines(xx, '../tables/table3.tex')
...
gt <- plot_covariate_means_by_group(
  .df,
  .title = NULL,
...
ggsave('../figures/figure3.pdf', 
       width = 6.64, height = 4.25)

xx <- as.matrix(apply(gt$data[,
                              c('estimate', 'se', 'covariate', 'leaf')], 
            2, function(x) gsub('\n', ' ', 
                    x,
                     perl = TRUE,
                     )))
write.csv(x = xx,
          file = '../figures/figure3_source.csv', 
          row.names = FALSE
)
...
ggplot(data = xx[which(xx$`Total observations`<A),], aes(y = `Cumulative assignment counts`, 
  ...
  theme(panel.background = element_rect(fill = 'white'),
        plot.background = element_rect(fill = 'white', color = 'white'))
ggsave('../figures/respondent_base_assignment_on_time.pdf', width = 6, height = 4.5)
...
ggplot(data = xx[which(xx$`Total observations`<A),], aes(y = `Cumulative assignment counts`, 
  ...
  theme(panel.background = element_rect(fill = 'white'),
        plot.background = element_rect(fill = 'white', color = 'white'))
ggsave('../figures/headline_base_assignment_on_time.pdf', width = 6, height = 4.5)
...
ggsave('../figures/covariate_optimal_original.pdf', 
       width = 6.25, height = 4)
...
x_est <- rate$estimate*100
x_se <- rate$std.err*100
x_stat <- x_est/x_se
...
cat(
  paste0('estimate = $', round(x_est,1), '$~pp, ',
         's.e. = $', round(x_se,1), '$, ',
         '$Z = ', round(x_stat,2), '$, ',
         '$p ', ifelse(2*(1-pnorm(abs(x_stat)))< 0.001, '< 0.001', paste0('= ', round(2*(1-pnorm(abs(x_stat))),3) )) ,'$, ',
         '95\\% CI = ', '[$', paste(round(x_est + x_se*1.96*c(-1,1),2), collapse = '$, $'), '$]' )
)
...
ggsave('../figures/rate_accuracy_vs_facebook.pdf', height = 5, width = 8)

# Benefit at 30%

rate$TOC[which(rate$TOC$q == 0.3),]

x_est <- rate$TOC[which(rate$TOC$q == 0.3),'estimate']*100
x_se <- rate$TOC[which(rate$TOC$q == 0.3),'std.err']*100
x_stat <- x_est/x_se
...
if(file.exists('objects/aipw_scores_nopretest.RDS')){ # read in scores if already generated
  aipw_scores_nopretest <- readRDS('objects/aipw_scores_nopretest.RDS')
...
if(file.exists('objects/aipw_scores_nocovariates.RDS')){ # read in scores if already generated
  aipw_scores_nocovariates <- readRDS('objects/aipw_scores_nocovariates.RDS')
...
l0 <- bind_rows(tidy(l0)[2:4,], tidy(l0)[6,], tidy(l0l)[2,], tidy(l0a)[2,], 
                tidy(l0)[1,])
...
l1 <- bind_rows(tidy(l1)[2:4,], tidy(l1)[6,], tidy(l1l)[2,], tidy(l1a)[2,], 
                tidy(l1)[1,])
...
l2 <- bind_rows(tidy(l2)[2:4,], tidy(l2)[6,], tidy(l2l)[2,], tidy(l2a)[2,], 
                tidy(l2)[1,])
...
aipw_te_nopretest$term <- aipw_te_nocovariates$term <- 
  aipw_te_unbalanced$term <- aipw_te_imputed$term <- aipw_te_adjusted$term <-
  treatment_levels[-1]

aipw_est_nopretest$term <- aipw_est_nocovariates$term <- 
  aipw_est_unbalanced$term <- aipw_est_imputed$term <- aipw_est_adjusted$term <-
  treatment_levels[1]
...
rows <- as.data.frame(matrix(c('', '(1)', '(2)', '(3)', '(4)', '(5)', '(6)', 
                               '(7)', '(8)',
                               'row2', rep('', 8),
                               'row7', rep('', 8),
                               'row16', rep('', 8)),
...
optimal_assignment_c_alt <- ifelse(
  predict(cf.priority_c_alt, xs_eval)$predictions>0, 4, 5)
...
if(file.exists('objects/aipw_scores_c_alt.RDS')){ # read in scores if already generated
  ...
  aipw_scores_c_alt <- aw_scores_eval(
    xs = xs_eval,
    yobs = yobs_eval, 
    ws = as.factor(case_when(ws_eval == 1 ~ 1,
...
if(file.exists('objects/aipw_scores_any_true_c_alt.RDS')){ # read in scores if already generated
  aipw_scores_any_true_c_alt <- readRDS(
    'objects/aipw_scores_any_true_c_alt.RDS')
  
}else{
  
  aipw_scores_any_true_c_alt <- aw_scores_eval(
    xs = xs_eval,
    yobs = any_true, 
    ws = as.factor(case_when(ws_eval == 1 ~ 1,
                             ws_eval == optimal_assignment_c_alt ~ 2,
...
if(file.exists('objects/aipw_scores_any_false_c_alt.RDS')){ # read in scores if already generated
  aipw_scores_any_false_c_alt <- readRDS(
    'objects/aipw_scores_any_false_c_alt.RDS')
  
}else{
  
  aipw_scores_any_false_c_alt <- aw_scores_eval(
    xs = xs_eval,
    yobs = any_false, 
    ws = as.factor(case_when(ws_eval == 1 ~ 1,
                             ws_eval == optimal_assignment_c_alt ~ 2,
  ...
  saveRDS(aipw_scores_any_false_c_alt, 
          'objects/aipw_scores_any_false_c_alt.RDS')
...
if(file.exists('objects/aipw_scores_timeline_true_c_alt.RDS')){ # read in scores if already generated
  aipw_scores_timeline_true_c_alt <- readRDS(
    'objects/aipw_scores_timeline_true_c_alt.RDS')
  
}else{
  
  aipw_scores_timeline_true_c_alt <- aw_scores_eval(
    xs = xs_eval,
    yobs = timeline_true, 
    ws = as.factor(case_when(ws_eval == 1 ~ 1,
                             ws_eval == optimal_assignment_c_alt ~ 2,
...
if(file.exists('objects/aipw_scores_timeline_false_c_alt.RDS')){ # read in scores if already generated
  aipw_scores_timeline_false_c_alt <- readRDS(
    'objects/aipw_scores_timeline_false_c_alt.RDS')
  
}else{
  
  aipw_scores_timeline_false_c_alt <- aw_scores_eval(
    xs = xs_eval,
    yobs = timeline_false, 
    ws = as.factor(case_when(ws_eval == 1 ~ 1,
                             ws_eval == optimal_assignment_c_alt ~ 2,
...
if(file.exists('objects/aipw_scores_send_true_c_alt.RDS')){ # read in scores if already generated
  ...
  aipw_scores_send_true_c_alt <- aw_scores_eval(
    xs = xs_eval,
    yobs = send_true, 
    ws = as.factor(case_when(ws_eval == 1 ~ 1,
                             ws_eval == optimal_assignment_c_alt ~ 2,
  ...
  saveRDS(aipw_scores_send_true_c_alt, 
          'objects/aipw_scores_send_true_c_alt.RDS')
  
}

if(file.exists('objects/aipw_scores_send_false_c_alt.RDS')){ # read in scores if already generated
  aipw_scores_send_false_c_alt <- readRDS(
    'objects/aipw_scores_send_false_c_alt.RDS')
  
}else{
  
  aipw_scores_send_false_c_alt <- aw_scores_eval(
    xs = xs_eval,
    yobs = send_false, 
    ws = as.factor(case_when(ws_eval == 1 ~ 1,
                             ws_eval == optimal_assignment_c_alt ~ 2,
  ...
  saveRDS(aipw_scores_send_false_c_alt, 
          'objects/aipw_scores_send_false_c_alt.RDS')
...
rows <- as.data.frame(matrix(c('', '\\textbf{Sharing}', '', '\\textbf{False}', 
                               '', '', '\\textbf{True}', '',
                               '', '\\textbf{Discernment}', 'Any sharing', 
                               'Messenger', 'Timeline', 'Any sharing', 
                               'Messenger', 'Timeline',
                               'row3', rep('', 7)),
                             byrow = TRUE, ncol = 8
))
...
df$`Logged negative ratio of false to true weights` <- log(-df$false_w/
                                                             df$true_w)
df$Statistic <- df$estimate/df$std.error
ggplot(df, aes(x = `Logged negative ratio of false to true weights`, 
               y = `Statistic`, color = Condition)) + 
  geom_point(size = 0.5) +
...
ggsave('../figures/alternative_weighting.pdf', width = 8, height = 4)
```
...
df_eval$pooled_respondent_timeline_false <- rowMeans(
  aipw_scores_timeline_false[, 4:5])
df_eval$pooled_respondent_timeline_true <- rowMeans(
  aipw_scores_timeline_true[, 4:5])
...
table <- sapply(covariate_list, function(x) {
  ...
  if(length(unique(df_eval[, covariate])) <= 2){
    ...
    # continuous covariates
    adjustment <--1e-5*c(2*(median(df_eval[,covariate])==
                              quantile(df_eval[,covariate], 1))-1)
    fmla0 <- formula(paste0(outcome, ' ~  factor(',covariate, 
                            '> median(df_eval$', covariate, ') +', adjustment,
                            ') -1'))
    fmla1 <- formula(paste0(outcome, ' ~  factor(',covariate, 
                            '> median(df_eval$', covariate, ') +', adjustment,
                            ')'))
    ns <- table(df_eval[,covariate] > median(df_eval[,covariate]) + adjustment)
  }
...
table <- apply(apply_mat, 1, function(x) {
  ...
  if(length(unique(df_eval[, covariate])) <= 2){
    ...
    adjustment <--1e-5*c(2*(median(df_eval[,covariate])==
                              quantile(df_eval[,covariate], 1))-1)
    fmla0 <- formula(paste0(outcome,
                            ' ~  factor(',covariate, '> median(df_eval$', 
...
models <- list(
  `Sharing\nDiscernment` = list(tidy = table_combined),
  `False: Any sharing` = list(
    tidy = table_any[which(table_any$`Stimuli type` == 'False'),]),
  `False: Messenger` = list(
    tidy = table_channel[which(table_channel$`Stimuli type` == 'False' &
                                 table_channel$Channel == 'send'),]),
  `False: Timeline` = list(
    tidy = table_channel[which(table_channel$`Stimuli type` == 'False' &
                                 table_channel$Channel == 'timeline'),]),
  `True: Any sharing` = list(
    tidy = table_any[which(table_any$`Stimuli type` == 'True'),]),
  `True: Messenger` = list(
    tidy = table_channel[which(table_channel$`Stimuli type` == 'True' &
                                 table_channel$Channel == 'send'),]),
  `True: Timeline` = list(
    tidy = table_channel[which(table_channel$`Stimuli type` == 'True' &
                                 table_channel$Channel == 'timeline'),])
...
table <- sapply(covariate_list, function(x) {
  ...
  if(length(unique(df_eval[, covariate])) <= 2){
    ...
    adjustment <--1e-5*c(2*(median(df_eval[,covariate])==
                              quantile(df_eval[,covariate], 1))-1)
    fmla0 <- formula(paste0('aipw_scores[,4] ~  factor(',covariate, 
                            '> median(df_eval$', covariate, ') +', adjustment,
                            ') -1'))
    fmla1 <- formula(paste0('aipw_scores[,4] ~  factor(',covariate, 
                            '> median(df_eval$', covariate, ') +', adjustment,
...
table <- apply(apply_mat, 1, function(x) {
  ...
  if(length(unique(df_eval[, covariate])) <= 2){
    ...
    fmla0 <- formula(paste0('aipw_scores_any_',stimuli_type, 
                            ...
                            ') -1'))
    fmla1 <- formula(paste0('aipw_scores_any_',stimuli_type, 
                            '[,4] ~  factor(',covariate, '> median(df_eval$', 
                            covariate, ') +', adjustment,')'))
    ns <- table(df_eval[,covariate] > median(df_eval[,covariate]) + adjustment)
  }
  ols0 <- tidy(lm_robust(fmla0, data=df_eval))
...
table <- sapply(covariate_list, function(x) {
  ...
  if(length(unique(df_eval[, covariate])) <= 2){
    ...
    adjustment <--1e-5*c(2*(median(df_eval[,covariate])==
                              quantile(df_eval[,covariate], 1))-1)
    fmla0 <- formula(paste0('aipw_scores[,5] ~  factor(',covariate, 
                            '> median(df_eval$', covariate, ') +', 
                            adjustment,') -1'))
    fmla1 <- formula(paste0('aipw_scores[,5] ~  factor(',covariate, 
                            '> median(df_eval$', covariate, ') +', 
                            adjustment,')'))
...
table <- apply(apply_mat, 1, function(x) {
  ...
  if(length(unique(df_eval[, covariate])) <= 2){
    ...
    fmla0 <- formula(paste0('aipw_scores_any_',stimuli_type, 
                            ...
                            adjustment,') -1'))
    fmla1 <- formula(paste0('aipw_scores_any_',stimuli_type, 
                            '[,5] ~  factor(',covariate, '> median(df_eval$', 
                            covariate, ') +', adjustment,')'))
    ns <- table(df_eval[,covariate] > median(df_eval[,covariate]) + adjustment)
  }
  ols0 <- tidy(lm_robust(fmla0, data=df_eval))
...
table(df_learn$nigeria) + table(df_eval$nigeria)

# Users share more discerningly when they are nudged to consider information’s 
# accuracy, reducing misinformation sharing by

# Accuracy
# pp improvement
round(mean(aipw_scores_any_false[,4])/mean(aipw_scores_any_false[,1])*100, 1)
...
cat(
  paste0('estimate = $', round(x_est,1), '$~pp, ',
         's.e. = $', round(x_se,1), '$, ',
         '$Z = ', round(x_stat,2), '$, ',
         '$p ', ifelse(2*(1-pnorm(abs(x_stat)))< 0.001, '< 0.001', 
                       paste0('= ', round(2*(1-pnorm(abs(x_stat))),3) )) ,'$, ',
         '95\\% CI = ', '[$', 
         paste(round(x_est + x_se*1.96*c(-1,1),2), collapse = '$, $'), '$]' )
)
```

## Results: Study sample

```{r sample, cache = TRUE, warning=FALSE, message=FALSE}
table(df_learn$nigeria)
table(df_eval$nigeria)
```
...
cat(
  paste0('estimate = $', round(x_est,3), '$, ',
         ...
         '$p ', ifelse(2*(1-pnorm(abs(x_stat)))< 0.001, '< 0.001', 
                       paste0('= ', round(2*(1-pnorm(abs(x_stat))),3) )) ,'$, ',
         '95\\% CI = ', '[$', 
         paste(round(x_est + x_se*1.96*c(-1,1),3), collapse = '$, $'), '$]' )
)



write.csv(rbind(adaptive = c(mean(yobs_learn), 
                             sd(yobs_learn)/sqrt(length(yobs_learn))),
                uniform = est),
...
# ratio
1-round(mean(aipw_scores_timeline_false[,1])/mean(aipw_scores_send_false[,1]),
        3)


# men are more likely to share false posts than women, while low digital 
# literacy are more likely to share false posts
# Men
x_male <- table_any_control[which(table_any_control$`Covariate type` == 
                                    'Male' & 
                                    table_any_control$`Stimuli type` == 
                                    'False' & 
                                    grepl('Male', table_any_control$label)), ]

x_notmale <- table_any_control[which(table_any_control$`Covariate type` == 
                                       'Male' & 
                                       table_any_control$`Stimuli type` == 
                                       'False' & 
                                       grepl('Not male', 
                                             table_any_control$label)), ]

x_male_diff <- table_any_control[which(table_any_control$`Covariate type` == 
                                         'Male' & 
                                         table_any_control$`Stimuli type` == 
                                         'False' & 
                                         grepl('Difference', 
...
# ratio
round( (1-x_male[,'estimate']/x_notmale[,'estimate'])* 100, 1)

# confidence intervals, difference estimate
cat(
  paste0('estimate = $', round(x_male_diff[,'estimate']*100,1), '$~pp, ',
         's.e. = $', round(x_male_diff[,'std.error']*100,1), '$, ',
         '$Z = ', round(x_male_diff[,'estimate']/x_male_diff[,'std.error'],2), 
         '$, ',
         '$p ', ifelse(
           round(
             2*(1-pnorm(abs(x_male_diff[,'estimate']/
                              x_male_diff[,'std.error']))),2)< 0.001, '< 0.001', 
           paste0('= ', 
                  round(
                    2*(1-pnorm(abs(x_male_diff[,'estimate']/
                                     x_male_diff[,'std.error']))),2) )) ,
         '$, ',
         '95\\% CI = ', 
         '[$', 
         paste(
           round(
             x_male_diff[,'estimate']*100 +
               x_male_diff[,'std.error']*100*1.96*c(-1,1),2), 
           collapse = '$, $'), '$]' )
)

# DLI
x_highdli <- table_any_control[which(table_any_control$`Covariate type` == 
                                       'Digital literacy index' & 
                                       table_any_control$`Stimuli type` == 'False' & 
                                       grepl('Above', table_any_control$label)), ]

x_lowdli <- table_any_control[which(table_any_control$`Covariate type` == 
                                      'Digital literacy index' & 
                                      table_any_control$`Stimuli type` == 'False' & 
                                      grepl('Below', table_any_control$label)), ]

x_dli_diff <- table_any_control[which(table_any_control$`Covariate type` == 
                                        'Digital literacy index' & 
                                        table_any_control$`Stimuli type` == 'False' & 
                                        grepl('Difference', table_any_control$label)), ]

# ratio
round( (1-x_lowdli[,'estimate']/x_highdli[,'estimate'])* 100, 1)

# confidence intervals, difference estimate
cat(
  paste0('estimate = $', round(x_dli_diff[,'estimate']*100,1), '$~pp, ',
         's.e. = $', round(x_dli_diff[,'std.error']*100,1), '$, ',
         '$Z = ', round(x_dli_diff[,'estimate']/x_dli_diff[,'std.error'],2), '$, ',
         '$p ', ifelse(round(2*(1-pnorm(abs(x_dli_diff[,'estimate']/x_dli_diff[,'std.error']))),2)< 0.001, '< 0.001', paste0('= ', round(2*(1-pnorm(abs(x_dli_diff[,'estimate']/x_dli_diff[,'std.error']))),2) )) ,'$, ',
         '95\\% CI = ', '[$', paste(round(x_dli_diff[,'estimate']*100 + x_dli_diff[,'std.error']*100*1.96*c(-1,1),2), collapse = '$, $'), '$]' )
)
...
cat(
  paste0('estimate = $', round(x_est,3), '$, ',
         ...
         '$Z = ', round(x_stat,3), '$, ',
         '$p ', ifelse(2*(1-pnorm(abs(x_stat)))< 0.001, '< 0.001', paste0('= ', round(2*(1-pnorm(abs(x_stat))),3) )) ,'$, ',
         '95\\% CI = ', '[$', paste(round(x_est + x_se*1.96*c(-1,1),3), collapse = '$, $'), '$]' )
)
...
x_se <- sqrt(var(x_1)/length(x_1) + var(x_2)/length(x_2))*100
x_stat <- x_est/x_se
...
```{r pooled_respondent}
# men
x <- rowMeans(aipw_scores_any_false[(df_eval$male==1), 4:5])
x_est <- mean(x)*100
x_se <- sd(x)/sqrt(length(x))*100
x_stat <- x_est/x_se
...
# low dli
x <- rowMeans(aipw_scores_any_false[(df_eval$dli<=median(df_eval$dli)), 4:5])
x_est <- mean(x)*100
x_se <- sd(x)/sqrt(length(x))*100
x_stat <- x_est/x_se
...
x <- rowMeans(aipw_scores_any_false[(df_eval$science<=median(df_eval$science)), 4:5])
x_est <- mean(x)*100
x_se <- sd(x)/sqrt(length(x))*100
x_stat <- x_est/x_se
