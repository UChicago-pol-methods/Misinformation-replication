...
df <- within(df, {
  ...
  # Pledge variables
  pledge_public <- ifelse(!is.na(pledge_3a_post) & treatment_r == 'plege', 'public', 'private')
  pledge_seen <- as.numeric(pledge_3a_seen)
  pledge_post <- as.numeric(pledge_3a_post)
  ...
  post_true = case_when((incomplete %in% 1) & !(!is.na(dv_send_post8) & !is.na(dv_timeline_post8)) ~ as.integer(pre_true),
  ...
  post_false = case_when((incomplete %in% 1) & !(!is.na(dv_send_post8) & !is.na(dv_timeline_post8)) ~ as.integer(pre_false),
  ...
  pre_false_any[which(pre_false_any>=3)] <- 2
  ...
  post_false_send_prop = case_when(post_false_seen > 0 ~ post_false_send/ post_false_seen,
                                   TRUE ~ 0)
  # attrition
  attrited = case_when( (post_true_seen == 0 | post_false_seen == 0) ~ 1,
                        TRUE ~ 0)
}
)
```
...
df <- within(df, {
  ...
  pol_AB = 1 * (party %in% '1: jubilee') + 1 * (party %in% '1: apc')
  pol_AB[which(is.na(pol_AB))] = 0
}
)
...
# 1a. Predict m(x) = E[Y|X]
Y_hat_test <- predict(Y_forest, xs_h_new[h2,])$predictions[,1]
Y_hat_test_l <- predict(Y_forest, xs_h_new[-h2,])$predictions[,1]
# 1b. Predict W.hat = E[W|X] 
W_hat_test <- predict(W_forest, xs_h_new[h2,])$predictions
W_hat_test_l <- predict(W_forest, xs_h_new[-h2,])$predictions
# 1c. Predict baseline, E[Y(k) | X = x], and tau_k(X) = E[Y(k') - Y(k) | X = x]; 
# here accuracy is baseline, 
tau_hat_test <- predict(multi_forest, xs_h_new[h2,])$predictions[,,]
tau_hat_test_l <- predict(multi_forest, xs_h_new[-h2,])$predictions[,,]
Y_hat_baseline_test <- Y_hat_test - rowSums(W_hat_test[, -1, drop = FALSE] * tau_hat_test)
Y_hat_baseline_test_l <- Y_hat_test_l - rowSums(W_hat_test_l[, -1, drop = FALSE] * tau_hat_test_l)

# 2. combine to get mu_k(X) on test data (and re-order cols)
muk_test <- cbind(Y_hat_baseline_test, Y_hat_baseline_test + tau_hat_test)[,c(2, 1, 3:8)]
muk_test_l <- cbind(Y_hat_baseline_test_l, Y_hat_baseline_test_l + tau_hat_test_l)[,c(2, 1, 3:8)]
...
wopt_multi_forest_l <- WH_idx[[1]][levels_implement[apply(
  muk_test_l[,levels_implement], 1, which.max
)]]


prev_nrow <- nrow(df_treat)
df_holdout <- df_treat %>% 
  filter(batch == 5 & probs_0 ==0.025)

df_treat <- df_treat %>% 
  mutate(optimal0 = c(wopt_multi_forest_l, wopt_multi_forest),
         opt_col = paste0('probs_', optimal0-1)) %>%
  rowwise() %>%
  mutate(optimal_probs = get(opt_col)) %>%
  ungroup() %>%
  # mutate(optimal0 = case_when(batch !=5 ~ NA_real_,
  #                             TRUE ~ optimal0)) %>%
  filter(optimal_probs!=0) %>% 
  filter( (batch != 5) | ((probs_0 %in% c(0.1666666667))),
          Y >= -4 & Y <= 2,
          post_true >= 0 & post_true <= 4,
          post_false >= 0 & post_false <= 4, 
          pre_send_true >= 0 & pre_send_true <= 2, 
          pre_send_false >= 0 & pre_send_false <= 2, 
          post_timeline_true >= 0 & post_timeline_true <= 2,
          post_timeline_false >= 0 & post_timeline_false <= 2, 
          seen_false < 6 & seen_true < 6) %>% 
...
attr(df_treat$optimal0, "shortDescription") <- "Original optimal assignment policy."
...
true_names <- intersect(sapply(stimuli_T, function(x) paste0("share_", x)), colnames(df_treat))
false_names <- intersect(sapply(stimuli_F, function(x) paste0("link_", x)), colnames(df_treat))
dv_secondary <- colnames(df_treat)[grepl("dv_stimulus_", colnames(df_treat))]

CLEANED_FILENAME <- paste0('cleaned-data-AB_',
                           Sys.Date())
write_rds(cbind(df_treat[, c('ID','Y', 'W', context_cols, 'pol_AB', 'hhi_AB', paste0('probs_', 0:39))]),
          paste0('../data/', CLEANED_FILENAME,'.rds'))

CLEANED_FILENAME <- paste0('cleaned-data-secondary_',
                           Sys.Date())
write_rds(cbind(df_treat[, c('ID','Y', 'W', context_cols, true_names, false_names, dv_secondary, paste0('probs_', 0:39))]),
          paste0('../data/', CLEANED_FILENAME,'.rds'))

CLEANED_FILENAME <- paste0('cleaned-data_', 
                           Sys.Date())
write_csv(cbind(df_treat[, c('ID', 'Y', 'W', 'optimal0', context_cols, paste0('probs_', 0:39))]), 
          paste0('../data/', CLEANED_FILENAME,'.csv'))

write_rds(cbind(df_treat[, c('ID','Y', 'W', 'optimal0', context_cols, paste0('probs_', 0:39))]), 
          paste0('../data/', CLEANED_FILENAME,'.rds'))

# save user count
tmp <- read_csv('../data/user_count.csv')
tmp <- cbind(tmp, final = c( table(df_treat$batch==5)[[1]], table(df_treat$batch==5)[[2]]))
write.csv(tmp, '../data/user_count.csv', row.names = FALSE)
...
makeCodebook(df_treat[, c('Y', 'W', context_cols)], replace = TRUE, 
             ...
             file = '../data/infodemic_codebook.Rmd')
```
...
NCmisc::list.functions.in.file('dataCleaning_evaluation.R')
```
